...
export function AnimeRail({ label, title, items }: { label: string; title: string; items: Anime[] }) {
  return <View style={styles.section}><View style={styles.heading}><View style={styles.headingCopy}><Text style={styles.label}>{label}</Text><Text style={styles.title}>{title}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`Open ${title} catalog`} onPress={() => router.push("/catalog" as never)} style={styles.browse}><Text style={styles.browseText}>VIEW ALL</Text><Text style={styles.browseArrow}>↗</Text></Pressable></View><FlatList horizontal data={items} keyExtractor={(item) => String(item.id)} renderItem={({ item }) => <AnimeCard anime={item} />} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.rail} /></View>;
}
