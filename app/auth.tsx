...
import { readCallbackValue, signUpVerificationNotice } from "@/lib/auth-redirect";
...
export default function AuthScreen() {
  ...
  useEffect(() => {
    ...
    callback.then(({ error: exchangeError }) => {
      ...
      else { setMode("signIn"); setMessage("Email verified. Signing you in to Aniraku."); }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === "signUp") {
        ...
        await auth.signUp(email, password, username); setMode("signIn"); setPassword(""); setMessage(signUpVerificationNotice(email)); return;
      }
