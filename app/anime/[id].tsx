...
export default function AnimeDetailScreen() {
  ...
  return <NativeScreen>
    ...
    <View style={styles.summary}><Text style={styles.copy}>{(data.description || "No synopsis available.").replace(/<[^>]+>/g, "")}</Text>{data.genres?.length ? <Text style={styles.genre}>{data.genres.join(" · ")}</Text> : null}</View>
...
const styles = StyleSheet.create({
  ...
  genre: { color: nothing.white, fontSize: 10, lineHeight: 15, letterSpacing: 0.4 },
